...
- **表現規約**：
  - **ドメインモデル（集約）**：`timeRange: TimeRange` として値オブジェクトで表現。不変条件の検証をカプセル化
  - **シリアライゼーション（API/イベント）**：`startAt`、`endAt` として個別フィールドで表現。JSON互換性とスキーマの明確化のため
  - **永続化（DB）**：`start_at`、`end_at` として個別カラムで表現。インデックス効率とクエリの柔軟性のため
